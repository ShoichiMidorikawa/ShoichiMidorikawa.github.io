...
c0 = df_r.loc[:,"Date"]
c1 = df_r.loc[:,"Hokkaido"]

df0 = pd.DataFrame(data=c0)

for i in range(0, num):
    df0.loc[i,"Hokkaido"] = c1[i]
...
ax1.bar(df0["Date"],df0["Hokkaido"], label="北海道の感染者数_1日ごとの発表数",color='royalblue',width=0.85,align="center")
...
ax2.bar(df0["Date"],df0["死者数"], label="北海道の死者数_1日ごとの発表数", color='red',width=0.6,align="center")
