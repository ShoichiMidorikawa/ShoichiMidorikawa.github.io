...
for i in range(0, num-dayB):
    df0.loc[i,"実効再生産数B_d日後"] = c2[i+dayB]
...
for i in range(0, num-dayN):
    df0.loc[i,"実効再生産数N_d日後"] = c4[i+dayN]
...
ax2.plot(df0["Date"],df0["実効再生産数B_d日後"],":", label="R_B(t)を左に"+str(dayB)+"日移動", color='blue',linewidth=1.)
ax2.plot(df0["Date"],df0["実効再生産数N_d日後"],":", label="R_N(t)左に"+str(dayN)+"日移動", color='red',linewidth=1.)
