/*       Hyperboloid1.java (hyperboloid of 1 sheet 一葉双曲面)   */
/*       材質を設定                             　　　　　　　　 */
/*       インデックス付き4角形ポリゴン(完成)                     */
...
public class Hyperboloid1 extends Shape3D{
   ...
   private static Geometry createGeometry(){
    ...
    for(int j  = 0; j < m; j++){
       ...
       for(int i = 0; i < n; i++){
         int k = i + n*j;
         float x = (float)(a*Math.cos(theta)*Math.cosh(phi));
         float y = (float)(b*Math.sin(theta)*Math.cosh(phi));
         float z = (float)(c*Math.sinh(phi));
   ...
   private static Appearance createAppearance(){
    ...
    Color3f color3f = new Color3f(1.0f, 0.0f, 0.0f);  // 紫
