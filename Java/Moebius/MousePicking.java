...
/*----------------- シーングラフの構築　------------------*/
      ...
      TransformGroup transformGroup = new TransformGroup(); // new
      Transform3D transform3D = new Transform3D();          // new
      transform3D.setScale(0.65);                           // new スケールを0.65に縮小 
      transformGroup.setTransform(transform3D);             // new
...
/*------------------ Capabilityの設定 --------------------*/
      transformGroup.setCapability(MyObject.ALLOW_TRANSFORM_WRITE);  // change
      transformGroup.setCapability(MyObject.ALLOW_TRANSFORM_READ);   // change
      transformGroup.setCapability(MyObject.ENABLE_PICK_REPORTING);  // change
