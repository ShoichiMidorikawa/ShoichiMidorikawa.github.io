...
　　　  '$$ \\sin^2\\theta=\\left(\\frac{1}{2}-\\cos\\theta\\right)^2 $$'+
        '$$ \\sin^2\\theta=1-\\cos^2\\thetaより、 \\sin^2\\thetaに代入して、 $$'+
  ...
  var n5=0;  
      function display_eq5(){
         if(n5%2 == 0){ 
            ...
            '$$ 接する点を(x,y)とおくと、点A\\;(1,3)を通り、円\\;x^2+y^2=5に第2象限で接する直線の傾きは、 $$'+
            ...
            '$$ (y-1)(y-2)=0 $$'+
            '$$ y=1,2 $$'+
            '$$ y=1の場合\\;x+3y=5の式に代入して、 $$'+
            '$$ x+3=5 $$'+
            '$$ x=2 $$'+
            '$$ y=2の場合\\;x+3y=5の式に代入して、 $$'+
            '$$ x+6=5 $$'+
            '$$ x=-1 $$'+
            '$$ この条件を満たすのは　１　-x+2y=5 $$'+
            '$$  $$'+
            ...
            '$$ y-3=ax-a $$'+
            '$$ y=ax-(a-3) $$'+
            '$$ この直線の式を、円\\;x^2+y^2=5の式に代入して、 $$'+
            '$$ x^2+\\left\\{ax-(a-3)\\right\\}^2=0 $$'+
...
　　　  '$$ 三角関数の合成より、 $$'+
            '$$ \\sqrt{3}\\sin\\frac{\\pi}{12}+\\cos\\frac{\\pi}{12}=\\sqrt{(\\sqrt{3})^2+1^2} \\left( \\sin\\frac{\\pi}{12} ・\\frac{\\sqrt{3}}{\\sqrt{(\\sqrt{3})^2+1^2}}+\\cos\\frac{\\pi}{12} ・\\frac{1}{\\sqrt{(\\sqrt{3})^2+1^2}} \\right) $$'+
             '$$ =2\\left(\\sin\\frac{\\pi}{12}・\\cos\\frac{\\pi}{6}+\\cos\\frac{\\pi}{12}・\\sin\\frac{\\pi}{6}\\right) $$'+
            '$$ =2\\sin\\left(\\frac{\\pi}{12}+\\frac{\\pi}{6}\\right)=2\\sin\\frac{\\pi}{4}  $$'+
 ...
 var n15=0;  
      function display_eq15(){
         if(n15%2 == 0){ 
            ...
            '$$ PQ=PR+RQより、PR、RQにそれぞれ代入して、 $$'+
            '$$ PQ=\\frac{3}{\\tan\\alpha}+\\frac{3}{\\tan\\beta} $$'+
            '$$ PQ=3\\left(\\frac{1}{\\tan\\alpha}+\\frac{1}{\\tan\\beta}\\right) $$'+
            '$$ \\frac{1}{\\tan\\beta}=\\frac{1}{\\tan\\left(\\frac{\\pi}{2}-\\alpha\\right)}=\\tan\\alphaより、\\tan\\alphaを代入して、 $$'+
 ...
 var n18=0;  
      function display_eq18(){
         if(n18%2 == 0){ 
            ...
            '$$ 半径3-0=3、高さ3-2=1である。 $$'+
            '$$ yz平面で考え、\lは中心(0,0)、半径3の円の内部にあって、z軸と平行な線分の長さになる。 $$'+
            '$$ 中心(0,0)、半径3の円の方程式はz^2+y^2=9であるから、 $$'+
            '$$ z^2+t^2=9より、z=\\pm \\sqrt{9-t^2} $$'+
            '$$ よって\lの長さは、2\\sqrt{9-t^2} $$'+
            '$$ 三平方の定理より、r^2=3^2+(\\sqrt{9-t^2})^2=18-t^2 $$'+
            '$$ 面積S(t)は、半円rの円の面積から、半径2の円の面積を除いた部分である。 $$'+
            '$$ よって、S{(t)}=r^2\\pi-4\\pi=(18-t^2)\\pi-4\\pi=(14-t^2)\\pi $$'+
            '$$ これより体積Vは、 $$'+
